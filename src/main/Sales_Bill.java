...
if(barcode.getText().equals("")||quantity.getSelectedIndex()==0){
    ...
    JOptionPane.showMessageDialog(null,"The Remaining Quantity of this Drug is Zero\n\nYou should Update Quantity of this Drug","Warning",2);
        ...
        int requestedQuantity = Integer.parseInt(quantity.getSelectedItem().toString());
        if (availableQuantity <= 0) {
            JOptionPane.showMessageDialog(null, "The Remaining Quantity of this Drug is Zero\n\nYou should Update Quantity of this Drug", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (requestedQuantity > availableQuantity) {
            JOptionPane.showMessageDialog(null, "Requested Quantity exceeds Available Quantity", "Error", JOptionPane.ERROR_MESSAGE);
        } else {
        String Barcode = res.getString("BARCODE");
        String NAME = res.getString("NAME");
        String TYPE = res.getString("TYPE");
        String DOSE = res.getString("DOSE");
        double PRICE = res.getDouble("SELLING_PRICE");
        double AMOUNT = Integer.parseInt(quantity.getSelectedItem().toString()) * (PRICE);
